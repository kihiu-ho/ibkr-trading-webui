# Multi-Timeframe Chart Generation - Spec
...
{
    "symbol": str,           # e.g., "TSLA"
    "timeframe": str,        # "1d", "1w", "1mo"
    "period": int,           # Number of bars, default 200
    "indicators": list[str]  # Optional, default all
}
```
...
```python
{
    "symbol": str,
    "timeframe": str,
    "chart_url_jpeg": str,     # Public MinIO URL
    "chart_url_html": str,     # Interactive HTML URL
...
class ChartGenerator:
    ...
    def _fetch_market_data(symbol, timeframe, period) -> DataFrame
    def _calculate_indicators(df: DataFrame) -> dict
    def _create_plotly_figure(df, indicators, symbol) -> go.Figure
    def _export_to_image(fig: go.Figure) -> bytes
    def _upload_to_minio(image_data, filename) -> str
```

### Dependencies
- `plotly` - Chart creation
- `kaleido` - Image export
- `stock_indicators` or `pandas_ta` - Technical indicators
- Existing `MinIOService` - Chart storage
- Existing `IBKRService` - Market data
