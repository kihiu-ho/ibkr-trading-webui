...
### 4. Associate Symbols With Workflows

Tell the system which Airflow workflows should process each ticker:

1. Navigate to `http://localhost:8000/workflow-symbols`.
2. Click **Add Symbol** (or edit an existing one) and fill out the metadata on Step 1.
3. On **Select workflows**, multi-select every DAG that should process the symbol. Workflows already linked show up as badges in the main table.
4. Save and verify the API response (`GET /api/workflow-symbols`) now includes the `workflows[]` array for that ticker.

> **Tip:** Cadence is defined directly in Airflow; this UI only manages the symbol-to-workflow mapping.
